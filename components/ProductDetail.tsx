      ...
      <div className="max-w-4xl mx-auto px-2 sm:px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden p-4 sm:p-6 flex flex-col sm:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col justify-center">
            <div className="mb-4">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1 block">Cores e Modelos Disponíveis</span>
              <h1 className="text-lg sm:text-2xl font-black text-gray-900 leading-tight uppercase tracking-tight">{product.title}</h1>
            </div>
            
            <div className="flex items-center justify-between bg-gray-50 p-4 rounded-2xl border border-gray-100">
              <div>
                <span className="text-[10px] font-bold text-gray-400 uppercase">Menor preço via {store.name}</span>
                <div className="text-2xl font-black text-success-500">{product.estimatedPrice}</div>
              </div>
              <button 
                onClick={handleBuy}
                className="bg-success-500 text-white font-black px-6 py-3 rounded-xl text-xs uppercase tracking-widest shadow-lg shadow-success-100 active:scale-95 transition-all"
              >
                Ir à Loja
              </button>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div 
            onClick={handleBuy}
            className="bg-white rounded-2xl border-2 border-success-500/30 p-4 flex items-center justify-between cursor-pointer group hover:bg-success-50/20 transition-all shadow-sm"
          >
             <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full ${store.bg} flex items-center justify-center`}>
                   <svg className={`w-5 h-5 ${store.color}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
                </div>
                <div>
                   <div className="text-xs font-black text-gray-900">{product.estimatedPrice} <span className="text-[10px] text-gray-400 font-normal ml-1">à vista</span></div>
             ...
             <div className="flex items-center gap-2">
                ...
                <div className="w-10 h-10 bg-success-500 text-white rounded-lg flex items-center justify-center shadow-md">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
           ...
           <div className="relative h-24 w-full bg-gray-50/50 rounded-xl overflow-hidden flex items-end">
              <svg className="w-full h-full" viewBox="0 0 100 30" preserveAspectRatio="none">
                 <path 
                   d="M0,25 L10,22 L20,26 L30,20 L40,18 L50,22 L60,15 L70,18 L80,10 L90,12 L100,5" 
                   fill="none" 
                   stroke="#0ea5e9" 
                   strokeWidth="1" 
                 ...
                 />
                 <path 
                   d="M0,25 L10,22 L20,26 L30,20 L40,18 L50,22 L60,15 L70,18 L80,10 L90,12 L100,5 L100,30 L0,30 Z" 
                   fill="url(#gradient)" 
                   fillOpacity="0.1" 
                 />
                 <defs>
        ...
        <div className="space-y-4">
           ...
           <div className="bg-white border-l-4 border-brand-600 p-5 rounded-2xl shadow-sm text-gray-700 leading-relaxed font-medium text-sm">
              {loadingPitch ? (
        ...
        {/* Captura de Lead */}
        <div className="bg-brand-900 p-6 rounded-3xl shadow-xl text-white">
          {!isSaved ? (
            <div className="animate-fadeIn">
              <h4 className="text-sm font-black uppercase tracking-tight mb-2">Quer um preço ainda menor?</h4>
              <p className="text-[11px] text-gray-400 mb-4 font-medium">Avisaremos você por e-mail assim que este produto baixar de preço ou surgir um cupom.</p>
              <form onSubmit={async (e) => { e.preventDefault(); if(email) { await interestService.saveEmail(email, product.id, product.title); setIsSaved(true); } }} className="flex gap-2">
                <input required type="email" placeholder="Seu e-mail..." value={email} onChange={(e) => setEmail(e.target.value)} className="flex-1 bg-white/10 border border-white/5 p-3 rounded-xl text-xs outline-none focus:border-brand-600" />
                <button type="submit" className="bg-brand-600 text-white font-black px-4 py-3 rounded-xl uppercase text-[9px] tracking-widest">Monitorar</button>
              </form>
            </div>
          ) : (
            <div className="text-center py-2 animate-fadeIn flex items-center justify-center gap-2">
              <div className="w-6 h-6 bg-success-500 rounded-full flex items-center justify-center text-white"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></div>
              <span className="text-[10px] font-black uppercase tracking-widest">Alerta de Preço Ativado!</span>
            </div>
          )}
        </div>
             ...
             <div className="flex flex-col gap-2">
                {discoveryList.slice(0, visibleDiscoverItems).map(p => (
                  <button 
                    ...
                    className="flex items-center gap-3 bg-white border border-gray-100 p-3 rounded-2xl shadow-sm hover:shadow-md transition-all text-left w-full group active:scale-[0.98]"
                  >
                     <div className="w-16 h-16 bg-white rounded-xl flex items-center justify-center p-2 flex-shrink-0 border border-gray-50 overflow-hidden">
                     ...
                     <div className="flex-1 min-w-0">
                        <h4 className="text-[11px] font-black text-gray-900 line-clamp-1 uppercase tracking-tight mb-1">{p.title}</h4>
                        <div className="flex items-center justify-between">
                           <span className="text-[11px] font-black text-brand-600">{p.estimatedPrice}</span>
